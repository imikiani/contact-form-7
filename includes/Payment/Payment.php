	...
	public function after_send_mail( $cf7 ) {
		...
		if ( $enable == "1" ) {
			...
			if ( $active_gateway == 'IDPay' ) {
				...
				if ( empty( $amount ) ):
					?>
                    <a href="<?php echo get_option( 'siteurl' ) ?>"><?php _e( 'Go back to the site!' ) ?></a>
                    <html>
                    <head>
                        <meta http-equiv="Content-Type"
                              content="text/html; charset=utf-8"/>
                        <title><?php _e( 'Error in payment operation.', 'idpay-for-contact-form-7' ) ?></title>
                    </head>
                    <link rel="stylesheet" media="all" type="text/css"
                          href="<?php echo plugins_url( 'style.css', __FILE__ ) ?>">
                    <body>
                    <div>
                        <h3><?php _e( 'Error in payment operation.', 'idpay-for-contact-form-7' ) ?></h3>
                        <h4><?php _e( 'Amount can not be empty.', 'idpay-for-contact-form-7' ) ?></h4>
                    </div>
                    </body>
                    </html>
					<?php
				...
				if ( $http_status != 201 || empty( $result ) || empty( $result->id ) || empty( $result->link ) ):
                    ...
                    <div>
                        <h3><?php _e( 'Error in payment operation.', 'idpay-for-contact-form-7' ) ?></h3>
						<h4><?php echo sprintf(__(  'An error occurred while creating a transaction. error status: %s, error code: %s, error message: %s', $http_status, $result->error_code, $result->error_message  )) ?></h4>
                        <a href="<?php echo get_option( 'siteurl' ) ?>"><?php _e( 'Go back to the site!' ) ?></a>
                    </div>
                    </body>
                    </html>
